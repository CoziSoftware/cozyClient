...
public class WGonbleWareModule extends WPressable implements GonbleWareWidget {
    ...
    protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
        ...
        if (highlight) {
            ...
            renderer.quad(x, y, 2, height, hColor);

            renderer.quad(x - 2 + width, y, 2, height, hColor);
            renderer.quad(x, y - 2 + height, width, 2, hColor);
        }
        ...
        renderer.text(module.title, x, y + pad, color, false);
    }
}
